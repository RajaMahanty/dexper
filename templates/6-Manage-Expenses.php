			...
			<div class="card">
				...
				<div class="card-content">
					<table>
						...
						<tbody>
							<?php
							$totexp = $getFromE->allexpWithCategory($_SESSION['UserId']);
							if ($totexp !== NULL) {
								$len = count($totexp);
								for ($x = 1; $x <= $len; $x++) {
									echo "<tr>
											...
											<td>" . date("g:i A - d/m/Y", strtotime($totexp[$x - 1]->Date)) . "</td>
											<td>" . $totexp[$x - 1]->CategoryName . "</td>
											<td>
											<form style='margin-block-end: 0;' action='' method='post'>
												<input style='display:none;' name='ID' value=" . $totexp[$x - 1]->ID . "></input>
